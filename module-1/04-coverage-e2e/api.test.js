...
describe('API suite test', () => {
    ...
    describe('/login', () => {
        it("should login succesfully on the login route and return HTTP Status 200", async () => {
            const response = await request(app).post('/login').send({ username: "FelipeSantos", password: "123" }).expect(200);
            assert.deepStrictEqual(response.text, 'Logging has succeeded!');
        })

        it("should unauthorize a request when requesting it using wrong credentials and return HTTP Status 401", async () => {
            const response = await request(app).post('/login').send({ username: "wrong", password: "000" }).expect(401);
